...
export default function Jogo() {
  const [jogo, setJogo] = useState<EstadoJogo>(ESTADO_INICIAL);
  ...
  function handleSelecionar(pessoaId: number) {
    if (acertou) return;

    if (pessoaId === perguntaAtual.respostaId) {
      const novoTotal = jogo.pontos + PONTOS_POR_ACERTO;
      setJogo((prev) => ({ ...prev, pontos: novoTotal }));
  ...
  function proximaPergunta() {
    ...
    if (proximoIndice < jogo.perguntas.length) {
      setIndiceAtual(proximoIndice);
      setAcertou(false);
    } else {
      setFase(jogo.pontos === PONTUACAO_MAXIMA ? "venceu" : "perdeu");
    }
  }
      ...
      <div className="relative z-10 w-full max-w-xl">
        <div className="mb-8 flex items-center justify-between rounded-full border border-[#963BC4]/40 bg-[#160821]/80 px-6 py-3">
          <span className="text-sm text-[#B79CC9]">
            Pergunta {indiceAtual + 1} de {jogo.perguntas.length}
          </span>
          <span className="text-lg font-bold text-white">
            {jogo.pontos} <span className="text-[#963BC4]">pts</span>
          </span>
        </div>
        ...
        {acertou && (
          ...
          >
            {indiceAtual + 1 < jogo.perguntas.length ? "Próxima pergunta" : "Ver resultado"}
          </button>
        )}
      </div>
    </main>
